...
빈자리를 0으로 패딩하겠다는 뜻이다.  
문장 맨 뒤의 `":"` 떄문에 여기서는 무조건 XX:YY:ZZ: 형식으로 저장된다.  
...
- `std::string correct(std::string timeString)`  
문제에서 주어지는 원본 함수이다.  
여기에서 입력하는 시간문자열의 기본적인 형태를 검사한다.  
이를 위해 `regex` 정규표현식이 사용되었다.  
선언법은 아래와 같다.  
`std::regex e("\\d\\d\\:\\d\\d\\:\\d\\d")`
구체적인 문법은 Clever Code에서 설명되있다.  
이걸 제외하면 나머지는 위의 2개함수에서 일을 다 하기때문에 별 볼일이 없다.  
